...
bool Cell_Tumor::agent_state_step(double t, double dt, Coord& c){ 
	reset_PDL1();
	return false; 
};
...
void Cell_Tumor::setup_chem_source(BioFVMSinkSource*& s, chem_ID i, double rate){
	...
	//std::cout << "Adding chem source at: ( " << pos[0] << ", " << pos[1] << ", " << pos[2] << ")" << std::endl;
...
void Cell_Tumor::move_source_sink(BioFVMSinkSource* const s)const{
	if (s)
	{
		get_tumor().get_chem_grid().move_source_sink(s, getCoord());
	}
	return;
}
...
bool Cell_Tumor::is_PDL1_pos(void)const{
	...
	AgentType cell_type = getType();

	//Here, All T cells (including CD8+ and CD4+ T cells are PDL1 negative
	if (cell_type == AgentTypeEnum::CELL_TYPE_T || cell_type == AgentTypeEnum::CELL_TYPE_TCD4) {
		return false;
	}
	else {
		static double highPDL1 = params.getVal(PARAM_PDL1_HIGH_TH) * params.getVal(PARAM_PDL1_SYN_MAX);
		//std::cout << "highPDL1= " << highPDL1 << std::endl;
...
void Cell_Tumor::reset_PDL1(void){
	...
	//Here, we assume CD8+ T cell does not express PDL1, and two CD8+ T cell will not suppress each other.
	//Also, Treg does not express PDL1 as well, it suppress T cell via other method.
	if (cell_type == AgentTypeEnum::CELL_TYPE_T || cell_type == AgentTypeEnum::CELL_TYPE_TCD4) {
		_PDL1_syn = 0;
		//std::cout << "T cell detected: " << cell_type << ", with PDL1: " << _PDL1_syn << std::endl;
		return;
	}
	else {
		...
		/*std::cout << "Cell Type: "<< cell_type << ", IFNg: " << IFNg << ", PDL1_syn: " << _PDL1_syn <<
